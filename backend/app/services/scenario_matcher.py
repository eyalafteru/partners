...
כללים לתשובה:
1. התייחס ספציפית לבקשה/שאלה של הלקוח/ה
2. אם מבקשים משהו שאין לנו (כמו מחשבון פוריות) - יש להסביר בנימוס שאנחנו מתמחים במחשבונים פיננסיים (הלוואות, משכנתא, חיסכון, פנסיה)
3. תמיד להציע את המחשבונים הפיננסיים שלנו: https://loan-israel.co.il/category/כלים-ומחשבונים/
4. שמור על טון חברי ומקצועי
5. התשובה צריכה להיות קצרה וממוקדת
6. חובה: כתוב בשפה ניטרלית מבחינת מגדר - מתאימה לנשים ולגברים כאחד.
   השתמש בצורות סביליות/ניטרליות: "ניתן למצוא" במקום "תוכל למצוא", "אפשר לבדוק" במקום "תוכל לבדוק",
   "מוזמנים לפנות" במקום "מוזמן לפנות".
...
מנהל מקצועי | רק תבקש"""
        ...
        try:
            response, _ = await self.openai.generate(
                system_prompt="אתה עוזר שמייצר תשובות מקצועיות למיילים בעברית בשפה ניטרלית מגדרית. החזר JSON בלבד.",
